<x-layout-app page-title="Colaborator details">

    <div class="w-100 p-4">

        <h3>Colaborator details</h3>

        <hr>

        <div class="container-fluid">
            <div class="row mb-3">

                <div class="col">

                    <p>Name: <strong>{{ $colaborator->name }}</strong></p>
                    <p>Email: <strong>{{ $colaborator->email }}</strong></p>
                    <p>Role: <strong>{{ $colaborator->role }}</strong></p>
                    <p>Permissions: </p>

                    @php
                        $permissions =json_decode($colaborator->permissions);   
                    @endphp
                    <ul>
                        @foreach ($permissions as $permission)
                            <li>{{ $permission }}</li>
                        @endforeach
                    </ul>
                    <!-- permissions -->

                    <p>Department: <strong>{{ $colaborator->department->name }}</strong></p>
                    <p>Active: 
                        @empty($colaborator->email_verified_at)
                            <span class="badge bg-danger">No</span>
                        @else
                            <span class="badge bg-success">Yes</span>
                        @endempty
                    </p>
                </div>

                <div class="col">
                    <p>Address: <strong>{{ $colaborator->userDetail->address }}</strong></p>
                    <p>Zip code: <strong>{{ $colaborator->userDetail->zip_code }}</strong></p>
                    <p>City: <strong>{{ $colaborator->userDetail->city }}</strong></p>
                    <p>Phone: <strong>{{ $colaborator->userDetail->phone }}</strong></p>
                    <p>Admission date: <strong>{{ $colaborator->userDetail->admission_date }}</strong></p>
                    <p>Salary: <strong>{{ $colaborator->userDetail->salary }} $</strong></p>
                </div>
            </div>
        </div>

        <button class="btn btn-outline-dark" onclick="window.history.back()"><i class="fas fa-arrow-left me-2"></i>Back</button>

    </div>

</x-layout-app>